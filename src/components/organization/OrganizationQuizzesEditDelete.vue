    ...
    <div class="flex min-h-screen bg-gray-100 dark:bg-[#1a202c]">
        ...
        <div class="flex-1 flex flex-col md:ml-64 min-h-screen">
            ...
            <div class="flex-1 p-4">
                <!-- Page Header -->
                <div
                    class="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4 px-2 text-center sm:text-left">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-10">
                    <QuizCard v-for="quiz in paginatedData" :key="quiz.id" :quiz="quiz" @edit="editQuiz"
        ...
        <div v-if="showDeletePopup" class="fixed inset-0 z-[100] flex items-center justify-center bg-gray-600/75">
            <div class="bg-white dark:bg-gray-700 p-6 rounded-lg shadow-md mx-4 max-w-sm w-full">
                <p class="text-lg font-semibold text-center dark:text-white mb-4">Are you sure you want to delete this
                    quiz?</p>
                <div class="flex justify-center space-x-4">
                    <button @click="deleteQuiz"
                        class="bg-red-500 hover:bg-red-700 text-white px-4 py-2 rounded-lg cursor-pointer">Yes,
                        Delete</button>
                    <button @click="showDeletePopup = false"
                        class="bg-gray-300 hover:bg-gray-400 px-4 py-2 text-teal-800 rounded-lg cursor-pointer">Cancel</button>
                </div>
            </div>
        </div>
        <!-- Subscription Modal -->
