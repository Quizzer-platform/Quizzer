<template>
  <div class="flex min-h-screen w-full bg-gray-100">
      ...
      <main class="flex-1 p-6 w-full">
        ...
        <div v-else class="bg-white p-6 rounded-lg shadow-lg border border-gray-200 mt-6 w-full">
          <div class="flex flex-col md:flex-row md:justify-between md:items-center space-y-6 md:space-y-0">
            ...
            <div class="md:w-3/4">
              <h2 class="text-3xl font-bold text-teal-700">{{ selectedOrg.name }}</h2>
              <p class="text-gray-600 mt-1"><strong>ID:</strong> {{ selectedOrg.id || 'N/A' }}</p>
              <p class="text-gray-600"><strong>Admin Email:</strong> {{ selectedOrg.adminEmail || 'N/A' }}</p>
              <p class="text-gray-600"><strong>Admin Phone:</strong> {{ selectedOrg.adminPhone || 'N/A' }}</p>
            </div>

            <!-- Total Quizzes Card -->
            <div class="bg-gray-100 p-6 rounded-lg shadow-md w-full md:w-1/4 text-center border border-gray-200">
              <div class="flex flex-col items-center">
                <svg class="h-14 w-14 text-teal-600 mb-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4M9 17h6m2-10a2 2 0 00-2-2h-4a2 2 0 00-2 2H7a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2z"/>
                </svg>
                <p class="text-4xl font-bold text-teal-700">{{ selectedOrg.quizzes }}</p>
          ...
          <div class="mt-8 w-full">
            ...
            <div v-if="selectedOrg.plans && Object.values(selectedOrg.plans).length > 1" class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
              <div
            ...
            <p v-else class="text-gray-500 text-center mt-4">This organization has no additional plans yet.</p>
          </div>

          <!-- Created Quizzes Section -->
          <div class="mt-8 w-full">
            <div class="flex flex-col md:flex-row justify-between items-center mb-4">
    <h3 class="text-2xl font-bold text-gray-800">Created Quizzes</h3>
    ...
    />
            ...
            <!-- No Quizzes Message -->
            <div v-if="quizzes.length === 0" class="text-center text-gray-500 text-lg mt-6">
