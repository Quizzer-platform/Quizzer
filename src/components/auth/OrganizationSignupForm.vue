    ...
    <div class="flex items-center justify-center min-h-screen bg-gray-100 p-4">
        <div class="flex flex-col md:flex-row w-full max-w-4xl bg-white rounded-lg shadow-lg overflow-hidden">
            <div class="w-full md:w-1/2 p-6 md:p-10">
                <h2 class="text-2xl font-semibold text-teal-600 mb-6">Register Your Organization</h2>
                <form @submit.prevent="handleOrgSignup">
                    <div class="mb-4">
                        <label class="block text-gray-700">Organization Name</label>
                        <input v-model="org.name" type="text" placeholder="Enter organization name" 
                            class="w-full p-2 border rounded-lg">
                        <p v-if="errors.name" class="text-red-500 text-sm">{{ errors.name }}</p>
                    </div>
                    <div class="mb-4">
                        <label class="block text-gray-700">Admin Email</label>
                        <input v-model="org.email" type="email" placeholder="Enter admin email" 
                            class="w-full p-2 border rounded-lg">
                        <p v-if="errors.email" class="text-red-500 text-sm">{{ errors.email }}</p>
                    </div>
                    <div class="mb-4">
                        <label class="block text-gray-700">Admin Password</label>
                        <input v-model="org.password" type="password" placeholder="Enter admin password" 
                            class="w-full p-2 border rounded-lg">
                        <p v-if="errors.password" class="text-red-500 text-sm">{{ errors.password }}</p>
                    </div>
                    <div class="mb-4">
                        <label class="block text-gray-700">Admin Phone</label>
                        <input v-model="org.phone" type="text" placeholder="Enter admin phone number" 
                            class="w-full p-2 border rounded-lg">
