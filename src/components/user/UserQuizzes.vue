<template>
    <div class="flex flex-col min-h-screen bg-gray-100 text-gray-900 dark:bg-[#1a202c] dark:text-gray-100">
                        ...
                        <div v-else-if="userQuizzes.length > 0"
                            ...
                            <div v-for="quiz in userQuizzes" :key="quiz.quizId"
                                ...
                                <div class="flex items-center justify-between mb-2">
                                    <span class="text-sm font-semibold px-3 py-1 rounded-full bg-teal-600 text-white dark:bg-teal-700 dark:text-gray-200">
                                        {{ quiz.title }}
                                    </span>
                                ...
                                <h3 class="font-bold text-sm truncate">Description: {{ quiz.category || 'General' }}</h3>
                                <p class="text-sm mt-1">Questions: {{ quiz.totalQuestions }}</p>
                                <p class="mt-2 text-sm font-medium">
                                    Score:
                                    <span :class="quiz.quizScore >= quiz.totalQuestions / 2 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'"
                                        class="font-bold">
                                        {{ quiz.quizScore }} / {{ quiz.totalQuestions }}
                                    </span>
                                </p>
                                <button @click="goToQuizAnswers(quiz.quizId)"
                                    class="mt-4 bg-teal-700 hover:bg-teal-900 dark:bg-teal-700 dark:hover:bg-teal-700 text-white px-6 py-2 rounded-lg shadow-md transition">
        ...
        <div v-if="quizzesToTake === 0"
            ...
            <div class="bg-white dark:bg-gray-800 dark:text-gray-200 p-6 rounded-lg shadow-lg w-full max-w-md text-center">
