<template>
    <div class="bg-white p-6 rounded-xl shadow-xl mx-auto max-w-6xl 
               dark:bg-gray-800/95 dark:text-gray-200 backdrop-blur-sm">
            ...
            <table class="w-full text-left border-collapse overflow-hidden rounded-xl">
                <thead>
                    <tr class="bg-gradient-to-r from-teal-600 to-teal-700 text-white/95 
                        ...
                        <th v-for="(header, index) in headers" :key="index" 
                            class="p-4 font-medium tracking-wide text-sm uppercase 
                ...
                <tbody class="divide-y divide-teal-100/30 dark:divide-teal-900/50">
                    <tr v-for="(row, rowIndex) in rows" :key="rowIndex" 
                        class="hover:bg-teal-700/40 transition-all duration-150 ease-out bg-teal-50 dark:bg-teal-950/10
                               dark:hover:bg-teal-900/30 group">
                        <td v-for="(cell, colIndex) in row" :key="colIndex" 
                            class="p-4 text-gray-700 dark:text-teal-100 font-normal 
        ...
        <div class="block md:hidden space-y-3">
            <div v-for="(row, rowIndex) in rows" :key="rowIndex"
                class="bg-teal-100/50 backdrop-blur-sm rounded-xl p-4 shadow-sm ring-1 ring-gray-100/30
                ...
                <div v-for="(cell, colIndex) in row" :key="colIndex" 
                     class="flex justify-between items-center py-2.5 px-1
                ...
                <div v-if="showActions" class="pt-4 mt-3 border-t border-teal-100/30 
                    ...
                    <button class="w-full flex items-center justify-center gap-2 px-4 py-2.5 
                               ...
                               dark:hover:ring-teal-400/30 dark:text-teal-300"
                        @click="$emit('view-details', row[0])">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                                  d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
