<template>
    <main class="flex-1 p-6">
        ...
        <div v-else-if="subscriptions.length === 0"
            class="text-center bg-gray-100 dark:bg-gray-800 dark:text-gray-300 text-gray-700 p-4 rounded-lg shadow-md">
        ...
        <!-- Subscriptions Table -->
        <TableStructure v-else :headers="['Name of Plan', 'Quizzes', 'Price', 'Description']" :rows="paginatedData
            .filter(s => s.name && s.maxQuizzes && s.price && s.description)
            .map(s => [s.name, s.maxQuizzes, s.price, s.description])" :showActions="false" />

        <!-- Show No Data Message -->
        <p v-if="!loading && filteredSubscriptions.length === 0"
            class="text-center text-gray-500 dark:text-gray-400 mt-5 h-50 flex flex-col justify-center items-center">No
            subscriptions found</p>
